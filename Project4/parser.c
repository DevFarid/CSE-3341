...
void parseProcedure(struct nodeProcedure* p) {
	...
	if(currentToken() == AND) {
		...
		p->doesHaveFunctions = 1;
		p->funcs = calloc(1, sizeof(struct nodeFuncSeq));
		parseFuncSeq(p->funcs);
	} else {
		p->doesHaveFunctions = 0;
	}
...
void parseFuncSeq(struct nodeFuncSeq* fs) {
	fs->func = calloc(1, sizeof(struct nodeFunc));
	parseFunc(fs->func);
	...
	if(currentToken() == END) {
		...
		if(currentToken() == PROCEDURE) {
			fs->nextFunc = calloc(1, sizeof(struct nodeFuncSeq));
			parseFuncSeq(fs->nextFunc);
		}
	}
	// Otherwise, if nextToken() is not PROCEDURE, then we are done.
}

void parseFunc(struct nodeFunc* f) {
	char *paramId, **tempParamNamesBuffer;
	
	nextToken(); // consume PROCEDURE token
	expectedToken(ID);


	f->name = calloc(20, sizeof(char)); // allocate memory for function name
	paramId = calloc(20, sizeof(char)); // allocate memory for each parameter ID under the assumption each ID is 20 characters long
	getId(f->name);
	...
	f->numOfParamters = 0;
	f->parameters = calloc(1, sizeof(char*));

	while(currentToken() != RPAREN) {
		if(currentToken() == ID) {
			getId(paramId);
			tempParamNamesBuffer = realloc(f->parameters, (f->numOfParamters + 1) * sizeof(char*));
			
			if(tempParamNamesBuffer == NULL) {
				printf("Error: realloc failed in parseFunc during parsing parameters.\n");
				exit(1);
			}
	...
	nextToken(); // consume RPAREN
	expectedToken(IS); 
	nextToken(); // consume IS

	// being declaration of function
	f->ds = (struct nodeDeclSeq*) calloc(1, sizeof(struct nodeDeclSeq));
    parseDeclSeq(f->ds);

	// being statement of funciton
    expectedToken(BEGIN);
    nextToken();
...
void parseCall(struct nodeCall* c) {
	...
	expectedToken(ID);
	c->functionName = calloc(20, sizeof(char));
	
	getId(c->functionName); // get the function name of this call.
	nextToken(); // Consume ID
	...
	paramId = calloc(20, sizeof(char)); // allocate memory for each parameter ID under the assumption each ID is 20 characters long
	c->numOfParamters = 0;
	c->parameters = calloc(1, sizeof(char*));

	while(currentToken() != RPAREN) {
		if(currentToken() == ID) {
			getId(paramId);

			tempParamNamesBuffer = realloc(c->parameters, (c->numOfParamters + 1) * sizeof(char*));
			if(tempParamNamesBuffer == NULL) {
				printf("Error: realloc failed in parseCall during parsing parameters.\n");
				exit(1);
			}

			c->parameters = tempParamNamesBuffer;
			c->parameters[c->numOfParamters] = strdup(paramId);
